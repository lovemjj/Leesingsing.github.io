<template>
  <div class="zhen-duan-kai-fang">
    ...
    <div class="buttons" v-else>
      <van-button type="default" block @click="patchOrder">开始执行</van-button>
      <van-button type="danger" block v-if="info.assigned !== ''" @click="putOrder">转为公共服务单</van-button>
    </div>
  </div>
</template>
<script>
    ...
    patchOrder () {
      ...
      axios({
        ...
        if (res.data.code === 200) {
          ...
          t.$router.replace({ name: 'zhixingzhong', query: t.$route.query })
        } else {
          t.$notify({ message: res.data.message, type: 'warning' })
        }
      })
