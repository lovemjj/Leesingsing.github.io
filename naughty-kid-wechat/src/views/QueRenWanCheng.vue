<template>
  <div class="zhen-duan-kai-fang">
    ...
    <div class="info">
      ...
      <van-uploader v-model="info.signature" max-count="1" multiple />
    </div>
    <div class="info">
      ...
      <van-row style="width: 100%;">
        ...
        <van-col span="14"><van-rate v-model="info.satisfaction[0]" color="#ff9900" void-color="#ff9900" /></van-col>
      </van-row>
      <van-row style="width: 100%;">
        <van-col span="10">服务流程规范：</van-col>
        <van-col span="14"><van-rate v-model="info.satisfaction[1]" color="#ff9900" void-color="#ff9900" /></van-col>
      </van-row>
      <van-row style="width: 100%;">
        <van-col span="10">服务时间标准：</van-col>
        <van-col span="14"><van-rate v-model="info.satisfaction[2]" color="#ff9900" void-color="#ff9900" /></van-col>
      </van-row>
    </div>
    <div class="buttons">
      <van-button type="primary" block @click="patchOrder">家长确认完成</van-button>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    patchOrder () {
      const t = this
      if (t.info.signature.length === 0) {
        this.$notify({ message: '请上传家长/陪同人员签名（上传理疗单）', type: 'warning' })
        return true
      }
      t.$store.state.show = true
      axios({
        ...
        data: {
          signature: t.info.signature[0].content,
          satisfaction: t.info.satisfaction,
          status: 5
        }
      }).then((res) => {
        t.$store.state.show = false
