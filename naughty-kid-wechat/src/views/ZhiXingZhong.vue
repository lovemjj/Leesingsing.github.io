<template>
  <div class="zhen-duan-kai-fang">
    ...
    <div class="buttons" v-else>
      <van-button type="danger" block>终止执行</van-button>
      <van-button type="primary" block @click="patchOrder">执行完成</van-button>
    </div>
  </div>
</template>
<script>
    ...
    patchOrder () {
      ...
      axios({
        ...
        if (res.data.code === 200) {
          ...
          t.$router.replace({ name: 'querenwancheng', query: t.$route.query })
        } else {
          t.$notify({ message: res.data.message, type: 'warning' })
        }
      })
